...
def event():
    ...
    for i in range(len(sort_new)):
        if sort_new.loc[i, "성별"] == "남":
            team_arr.sort(key=lambda x: x[4])
    ...
    statistics = pd.DataFrame(columns = ["팀 번호","남", "여", "나이 평균", "학부 수", "사람 수"])
    ...
    for i in range(len(sort_result)):
        ...
        if i == 0:
            ...
            print(total_dep.loc[dep_count-1, "학부"])
            ...
            statistics.loc[cur_team, "사람 수"] = team_arr[cur_team][4]
